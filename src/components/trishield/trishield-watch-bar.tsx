...
export function TriShieldWatchBar({
  session,
  sticky = true,
}: {
  session: WatchSessionRow | null;
  sticky?: boolean;
}) {
  return (
    <div
      className={`${sticky ? "sticky top-0" : ""} z-40 -mx-4 mb-4 grid grid-cols-[minmax(0,1fr)] gap-3 bg-slate-900 px-4 py-2 text-white shadow-lg sm:-mx-6 sm:grid-cols-[minmax(0,1fr)_auto] sm:px-6`}
    >
      ...
      <div className="grid min-w-0 grid-cols-1 gap-2 sm:ml-auto sm:flex sm:flex-wrap sm:items-center sm:justify-end">
        <Party
          ...
          joinedAt={session?.session_started_at}
        />
        <Party
          label="Admin"
          count={session?.admin_snapshot_count ?? 0}
          active={!!session?.admin_camera_active}
          joinedAt={session?.admin_joined_at}
        />
        <Party
          label="SuperAdmin"
          count={session?.superadmin_snapshot_count ?? 0}
          active={!!session?.superadmin_camera_active}
          joinedAt={session?.superadmin_joined_at}
        />
      </div>
    </div>
  );
